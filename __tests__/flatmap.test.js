...
test('reduce a nested array 6 levels deep', function() {
	...
	expect(fm(arr)).toEqual([1,2,3,44,55,66,77,"hello", "inner",9999,"5 levels here we come", "6 levels woohoo!", 88,100]);
})
...
test('reduce a nested array 10 levels deep', function() {
	...
	expect(fm(arr)).toEqual([1,2,3,44,55,66,77,"5 levels here we come", "6 levels woohoo!", "7 levels", "8 levels", "9 levels", "10 levels", 88,100]);
})
...
test('reduce a nested object with callback', function() {
	...
	expect(fm(obj, ({x,y}) => Array.of(x,y))).toEqual([1,77,88,99,3,4,5,6]);
})
...
test('Using nested objects with a more unique callback.', function() {
	...
	expect(fm(obj, o => Object.values(o))).toEqual([1,77,88,99,'first','second', 3,4,5,6]);
})